...
var _transformContext = _interopRequireDefault(require("./helpers/transform-context"));

var _objectString = _interopRequireDefault(require("./helpers/object-string"));
...
class Compiler {
  compileSource(ast, options = {}) {
    ...
    return this.buffer('function template(__INIT__) {').indent().buffer('let __RESULT__ = $$.init(this, __INIT__)').visit(ast, '__RESULT__').buffer('return $$.end(__RESULT__)').undent().buffer('}');
  }
  ...
  compile(tree, options) {
    ...
    const {
      ...
      ast,
      map
    } = (0, _transformContext.default)(this.code, this.map);
  ...
  visitAttributes(tag, context) {
    ...
    if (Object.keys(ATTRIBUTES).length) {
      var attributes = (0, _objectString.default)(ATTRIBUTES);
    ...
    if (Object.keys(PROPERTIES).length) {
      var properties = (0, _objectString.default)((0, _flat.unflatten)(PROPERTIES));
    ...
    if (Object.keys(HOOKS).length) {
      const hooks = (0, _objectString.default)((0, _flat.unflatten)(HOOKS));
  ...
  visitMixin(mixin, context) {
    ...
    if (mixin.attrs.length) {
      ...
      attributes = (0, _objectString.default)(ATTRIBUTES);
    }
